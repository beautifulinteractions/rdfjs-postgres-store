...
class Store extends EventEmitter {
  ...
  _addWhereClausesForFilter(query, termName, filter) {
    ...
    const comparate_float = utils.parseTermToFloat(comparate);
    if (!comparate_float && comparate_float !== 0) {
      throw new Error(`Could not convert term "${comparate.value}" of type "${comparate.datatype.value}" to float`);
    }
    const operator = ({ gt: '>', lt: '<', gte: '>=', lte: '<=', eq: '=', neq: '!=' })[test];
    if (!operator) {
      throw new Error(`Unsupported test "${test}" in filter.`);
    }
    query.where(`${termName}_float`, operator, comparate_float);
    query.where(`${termName}`, 'LIKE', `%^^${comparate.datatype.value}`);
  }
    ...
    return this._knex.transaction(async (trx) => {
      return trx.schema
        .createTableIfNotExists('quads', (quadsTable) => {
          quadsTable.string('subject').notNullable();
          quadsTable.float('subject_float');
          quadsTable.string('predicate').notNullable();
          quadsTable.string('object').notNullable();
          quadsTable.float('object_float');
