...
function serializeQuad(quad) {
  ...
  const parsedSubjectValue = parseLiteralValue(quad.subject);
  if(parsedSubjectValue) {
    serialized[`subject_${parsedSubjectValue.type}`] = parsedSubjectValue.value;
  }
  const parsedObjectValue = parseLiteralValue(quad.object);
  if(parsedObjectValue) {
    serialized[`object_${parsedObjectValue.type}`] = parsedObjectValue.value;
  }
  return serialized;
}
